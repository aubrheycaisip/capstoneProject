...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Store</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["store_id"];
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		...
		$id = $_POST["brand_id"];
		$sql = "SELECT * FROM products WHERE product_store = '$id'";
	}else {
				...
				while ($row=mysqli_fetch_array($query)) {
					...
					echo 
						'<div class="row">
								...
								<div class="col-md-2"><input type="text" class="form-control price" value="'.$product_price.'"></div>
								<div class="col-md-2"><input type="text" class="form-control total" value="'.$product_price.'"></div>
							</div>';
				}
				...
				if (!isset($_SESSION["uid"])) {
					...
					//echo '
						...
						//	while($row=mysqli_fetch_array($query)){
							...
							//echo   
								//'<input type="hidden" name="return" value="http://localhost/capstoneproject/payment_success.php"/>
					            //   <input type="hidden" name="notify_url" value="http://localhost/capstoneproject/payment_success.php">
								//	<input type="hidden" name="cancel_return" value="http://localhost/capstoneproject/cancel.php"/>
